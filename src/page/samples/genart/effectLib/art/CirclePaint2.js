...
export function CirclePaint2(params = { nNoBG: true, nLineWidthFactor: .35, nX: 0, nY: 0, nRadiusFactor: 1.0 }) {
    let { nNoBG, nLineWidthFactor, nX, nY, nRadiusFactor } = params
    ...
    for (let j = 1; j < 180; j++) {
        for (let i = 0; i < 360; i++) {
            ...
            ctx3.moveTo(x, y)
            ctx3.lineTo(x2, y2)
            ctx3.arc(x, y, brightness / 10 * nRadiusFactor, 0, Math.PI * 2, false);
            ctx3.arc(x2, y2, brightness / 10 * nRadiusFactor, 0, Math.PI * 2, false);
            ctx3.stroke()
            ctx3.closePath()
        }
    }
